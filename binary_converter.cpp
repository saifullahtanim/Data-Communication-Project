...
// Set color (optional, but keeps important parts highlighted)
...
string bitStuffing(const string &binary) {
    ...
    for (char bit : binary) {
        ...
        if (bit == '1') {
            onesCount++;
            if (onesCount == 5) {
                stuffed += '0';  // Stuff a '0' after 5 consecutive 1's
                onesCount = 0;
            }
        } else {
            onesCount = 0;
...
string bitDestuffing(const string &stuffed) {
    ...
    for (char bit : stuffed) {
        ...
        if (bit == '1') {
            onesCount++;
            if (onesCount == 5) {
                onesCount = 0;  // Skip the stuffed '0'
                continue;
            }
        } else {
            onesCount = 0;
...
int main() {
    ...
    cout << "  Course Code: CSE 302 Section: 222 D1\n";
    cout << "=======================================\n";
